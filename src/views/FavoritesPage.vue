<template>
  <div class="absolute mt-2 ml-[10px] w-[130px] h-[100px] bg-cover bg-center" style="background-image: url('src/assets/gJW40PvAOx16R44UE4-ezgif.com-video-to-gif-converter.gif');"></div>
  <div class="max-w-10xl space-y-8 p-10 bg-white rounded-xl shadow-lg z-10">
    ...
    <section class="border-2 border-dashed border-gray-400 p-5 rounded-xl">
      ...
      <div v-if="filteredFavorites.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div v-for="tab in filteredFavorites" :key="tab.id" class="bg-orange-100 rounded-lg shadow-lg p-6 border-2 border-dashed border-gray-600 relative">
          <img class="mx-auto w-full block h-40 rounded-lg" alt="art cover" loading="lazy" src='https://picsum.photos/seed/2/2000/1000' />
          <div class="group">
            <button @click="removeFavorite(tab.id)" class="absolute top-1 right-2 z-99 text-red-500 hover:text-red-700 transform transition-transform duration-300 hover:rotate-90">✖</button>
          ...
          <div class="flex justify-start gap-2 items-center">
            <router-link :to="'/view/' + tab.id" class="hover:bg-red-400 bg-red-500 text-white cursor-pointer px-3 py-1 text-center rounded-2xl">View Tab</router-link>
            <div class="group">
              <button @click="toggleFavorite(tab)" class="px-3 py-1 rounded-2xl sm:w-auto sm:ml-auto" :class="[isFavorite(tab.id) ? 'bg-orange-400 shadow-lg text-white cursor-pointer' : 'text-gray-700 hover:bg-orange-500 bg-orange-400 shadow-lg hover:text-white cursor-pointer']">
                <i :class="isFavorite(tab.id) ? 'fa-solid fa-heart text-red-500' : 'fa-regular fa-heart'"></i>
              </button>
...
<script>
import { addToFavorites, removeFromFavorites, getAllFavorites } from '../services/tabService';
...
export default {
  ...
  methods: {
    ...
    async toggleFavorite(tab) {
      try {
        if (this.favorites.some(fav => fav.id === tab.id)) {
          removeFromFavorites(tab.id);
        } else {
          addToFavorites(tab.id);
        }
        this.favorites = getAllFavorites(); // Update favorites list
      } catch (error) {
  ...
  mounted() {
    this.favorites = getAllFavorites(); // Load favorites on page load
  },
};
</script>
